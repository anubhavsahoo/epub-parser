...
export default class Epub {
	...
	getMetadata(){
	  ...
	  simpleMeta.push({uniqueIdentifier})
	  simpleMeta.push({epubVersion})
	  simpleMeta.push({isEpub3})
	        ...
	        if(typeof metas[prop][0].$.id) {
	          if(metas[prop][0].$.id==uniqueIdentifier) {
	            if(typeof content == 'object') {
	              ...
	              const uniqueIdentifierValue = content;
	              simpleMeta.push({uniqueIdentifierValue});
	              if(typeof metas[prop][0].$.scheme !== 'undefined') {
	                uniqueIdentifierScheme = metas[prop][0].$.scheme;
	                simpleMeta.push({uniqueIdentifierScheme});
	              }               
	            }
	          }
	        };
